package com.jaziel.crawler.process.parse;
...
import com.jaziel.model.crawler.core.label.HtmlStyle;
 ...
 * * <p>
 * * <p>
 * * Pipeline负责抽取结果的处理，包括计算、持久化到文件、数据库等。WebMagic默认提供了“输出到控
 * 制台”和“保存到文件”两种结果处理方案。
 * * <p>
 * * Pipeline定义了结果保存的方式，如果你要保存到指定数据库，则需要编写对应的Pipeline。对于一
 * 类需求一般只需编写一个Pipeline。
 */
@Log4j2
public abstract class AbstractHtmlParsePipeline<T> extends AbstractProcessFlow implements Pipeline {
    ...
    public void process(ResultItems resultItems, Task task) {
        ...
        if(!CrawlerEnum.DocumentType.PAGE.name().equals(documentType)){
            log.info("不符合文档类型,url:{},docmentType:{},handelType:{}",url,documentType,handelType);
            return;
        }
