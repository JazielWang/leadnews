package com.jaziel.crawler.downloader;

import com.jaziel.crawler.factory.CrawlerProxyFactory;
 ...
 * 是从https://gitee.com/flashsword20/webmagic/blob/master/webmagiccore/
 * src/main/java/us/codecraft/webmagic/downloader/HttpClientDownloader.java
 * 获取源码后编辑的来的
 * <p>
...
public class ProxyHttpClientDownloader extends AbstractDownloader implements
    ...
    private HttpUriRequestConverter httpUriRequestConverter = new
            HttpUriRequestConverter();
    private ProxyProvider proxyProvider;
    private boolean responseHeader = true;

    public void setHttpUriRequestConverter(HttpUriRequestConverter
                                                   httpUriRequestConverter) {
    ...
    public Page download(Request request, Task task) {
        ...
        log.info("开始下载页面数据，url:{},handelType:{}",
                request.getUrl(), handelType);
...
//下载失败
            ...
            if (downloadStatus) {
                ...
                onSuccess(request);
                log.info("下载数据成功，url:{}，handelType:{},耗时：{}",
                        request.getUrl(), handelType, System.currentTimeMillis() - currentTime);
            } else {
                onError(request);
                log.error("下载文档失败，url:{},handelType:{},proxy:{},状态码：{}",
                        page.getUrl().toString(), handelType, proxy, page.getStatusCode());
            }
            return page;
        } catch (IOException e) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            crawlerHtml =
                    seleniumClient.getCrawlerHtml(page.getUrl().toString(), proxy,
                            cookieHelper.getCookieName());
            log.info("尝试使用selenium下载数据第{}次完成，代理：{}，url:{}，耗时：{}",
                    i, proxy, page.getUrl(), System.currentTimeMillis() - currentTime);
